...
-- 3. Fix user_id type inconsistency in device_registrations and user_sessions
-- Change from text to uuid for proper foreign key relationships
...
-- Update their RLS policies to use proper uuid comparison
...
-- 5. Remove role from profiles table to prevent self-promotion
...
-- 8. Create initial admin user role for the current authenticated user
-- This will need to be done manually after deployment