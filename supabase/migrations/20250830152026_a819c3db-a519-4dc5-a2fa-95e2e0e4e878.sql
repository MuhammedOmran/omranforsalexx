-- إصلاح المشاكل الأمنية - نظام عمران للمبيعات
...
-- 3. إضافة دالة للتحقق من صحة البيانات الحساسة
...
-- إضافة المحفزات للمراقبة الأمنية
CREATE TRIGGER audit_employee_basic_info_access
  AFTER SELECT ON public.employee_basic_info
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_sensitive_data_access();

CREATE TRIGGER audit_employee_directory_access
  AFTER SELECT ON public.employee_directory
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_sensitive_data_access();

-- 4. تحسين دالة التحقق من صلاحيات المدير للقسم
...
-- 5. إضافة دالة للتحقق من الأذونات المتقدمة
...
-- 6. تحديث trigger منع تصعيد الصلاحيات
...
BEGIN
  ...
  IF current_user_role = 'admin' THEN
    ...
    INSERT INTO public.advanced_security_logs (
      ...
      'access_control',
      'high',
      'تم تغيير دور المستخدم بواسطة المدير',
      jsonb_build_object(
  ...
  -- منع المستخدمين العاديين من تغيير الأدوار
  IF OLD.role IS DISTINCT FROM NEW.role THEN
    -- تسجيل محاولة تصعيد غير مصرح بها
    INSERT INTO public.advanced_security_logs (
      user_id, event_type, event_category, severity, description,
      metadata, success
    ) VALUES (
      ...
      'security_violation',
      'critical',
      'محاولة تصعيد صلاحيات غير مصرح بها',
      jsonb_build_object(
