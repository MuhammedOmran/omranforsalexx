...
-- تحديث جدول الإشعارات مع المزيد من الميزات
...
ADD COLUMN IF NOT EXISTS channels notification_channel[] DEFAULT ARRAY['in_app'],
...
CREATE TABLE IF NOT EXISTS notification_templates (
  ...
  email_template TEXT,
  sms_template TEXT,
  default_channels notification_channel[] DEFAULT ARRAY['in_app'],
...
CREATE TABLE IF NOT EXISTS notification_rules (
  ...
  name TEXT NOT NULL,
  description TEXT,
  rule_type TEXT NOT NULL, -- threshold, schedule, event_based
...
BEGIN
    CASE 
        WHEN pattern = 'daily' THEN
            next_time := current_time + INTERVAL '1 day';
        WHEN pattern = 'weekly' THEN
            next_time := current_time + INTERVAL '1 week';
        WHEN pattern = 'monthly' THEN
            next_time := current_time + INTERVAL '1 month';
        WHEN pattern LIKE 'every_%_minutes' THEN
            next_time := current_time + (SUBSTRING(pattern FROM 'every_(\d+)_minutes')::INTEGER || ' minutes')::INTERVAL;
        WHEN pattern LIKE 'every_%_hours' THEN
            next_time := current_time + (SUBSTRING(pattern FROM 'every_(\d+)_hours')::INTEGER || ' hours')::INTERVAL;
        ELSE
            next_time := current_time + INTERVAL '1 day';
    END CASE;
...
CREATE TRIGGER update_scheduled_notifications_updated_at BEFORE UPDATE ON scheduled_notifications FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
...
INSERT INTO notification_templates (user_id, name, type, subject_template, body_template, email_template, sms_template, default_channels) 
SELECT 
    auth.uid(),
    'فاتورة مستحقة',
    'invoice_due',
    'تذكير: فاتورة مستحقة رقم {{invoice_number}}',
    'عزيزي العميل، فاتورة رقم {{invoice_number}} بمبلغ {{amount}} مستحقة في {{due_date}}',
    'عزيزي العميل،<br><br>نود تذكيرك بأن فاتورة رقم {{invoice_number}} بمبلغ {{amount}} ج.م مستحقة الدفع في {{due_date}}.<br><br>يرجى سداد المبلغ في أقرب وقت ممكن.<br><br>شكراً لتعاونكم',
    'تذكير: فاتورة {{invoice_number}} بمبلغ {{amount}} ج.م مستحقة في {{due_date}}',
    ARRAY['in_app', 'email']
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE name = 'فاتورة مستحقة' AND user_id = auth.uid());