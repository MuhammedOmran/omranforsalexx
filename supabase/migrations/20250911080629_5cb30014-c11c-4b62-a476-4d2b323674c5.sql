-- إنشاء دالة لحساب معاملة مالية عند تغيير مخزون المنتج
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        IF stock_change != 0 THEN
            ...
            -- إذا نقص المخزون (بيع أو تلف)
            ELSIF stock_change < 0 THEN
                -- لا نسجل مصروف عند نقص المخزون، فقط عند الشراء
                -- يمكن تسجيل إيراد من البيع في مكان آخر
                NULL;
            END IF;
        END IF;
...
-- إنشاء trigger لتشغيل الدالة عند تغيير المنتجات
...
-- دالة لحساب معاملة مالية للمبيعات من الفواتير
...
DROP TRIGGER IF EXISTS invoices_cash_trigger ON invoices;
CREATE TRIGGER invoices_cash_trigger
    AFTER INSERT OR UPDATE ON invoices
    FOR EACH ROW
    EXECUTE FUNCTION handle_sales_cash_transaction();

-- دالة لحساب معاملة مالية لفواتير الشراء
...
-- إنشاء trigger لفواتير الشراء
DROP TRIGGER IF EXISTS purchase_invoices_cash_trigger ON purchase_invoices;
CREATE TRIGGER purchase_invoices_cash_trigger
    AFTER INSERT OR UPDATE ON purchase_invoices
    FOR EACH ROW
    EXECUTE FUNCTION handle_purchase_cash_transaction();